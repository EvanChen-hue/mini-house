...
function initData(vueOptions, context) {
  ...
  if (typeof data === 'function') {
    try {
      ...
      if (Object({"NODE_ENV":"development","VUE_APP_DARK_MODE":"false","VUE_APP_NAME":"大拇指家政","VUE_APP_PLATFORM":"mp-weixin","BASE_URL":"/"}).VUE_APP_DEBUG) {
...
var _default = {
  // toast提示框
  // 标题，图标，存在时间
  toastTip: function toastTip(title, icon, time) {
    uni.showToast({
  ...
  // 乘法
  mathMul: function mathMul(num1, num2) {
 ...
 */
var baseUrl = 'https://requests.taiyang.chat';
<<<<<<< HEAD
var REQUEST_TIMEOUT = 15000;
var loginRedirecting = false;
var parseUniRequestResult = function parseUniRequestResult(result) {
  if (Array.isArray(result)) {
    return {
      err: result[0],
      res: result[1]
...
var createRequest = function createRequest(opts, data) {
  var showLoading = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  var type = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'application/json';
  if (showLoading) {
    uni.showLoading({
      title: '加载中...'
    });
...
>>>>>>> b82c8976e75deb4638d7c2d0dbfdc908a3704f8d
  ...
  return new Promise(function (resolve, reject) {
<<<<<<< HEAD
    uni.request(httpDefaultOpts).then(function (result) {
      if (showLoading) {
        uni.hideLoading();
      }
      var _parseUniRequestResul = parseUniRequestResult(result),
        requestErr = _parseUniRequestResul.err,
        res = _parseUniRequestResul.res;
      if (requestErr) {
        reject(requestErr);
        return;
      }
      ...
      var httpOk = Number(res.statusCode) >= 200 && Number(res.statusCode) < 300;
=======
    uni.request(httpDefaultOpts).then(function (res) {
      if (showLoading) {
        uni.hideLoading();
      }
      var httpOk = res.statusCode === 200;
>>>>>>> b82c8976e75deb4638d7c2d0dbfdc908a3704f8d
      var bizCode = res && res.data && typeof res.data.code !== 'undefined' ? Number(res.data.code) : 200;
      var bizOk = bizCode === 200;
      var bizMsg = res && res.data && res.data.msg ? res.data.msg : '';
      if (httpOk && bizOk) {
        resolve(res.data);
        return;
      }
...
<<<<<<< HEAD
      var errMsg = response && response.errMsg || response && response.message || '';
      var isCode10 = String(errMsg).indexOf('code 10') !== -1 || String(errMsg).indexOf('Failed to fetch') !== -1;
      if (isCode10) {
